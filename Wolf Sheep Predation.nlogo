...
breed [ sheep_a a-sheep_a ]  ; sheep is its own plural, so we use "a-sheep" as the singular
...
to setup
  ...
  create-sheep_a initial-number-sheep_a  ; create the sheep a, then initialize their variables
  [
    set shape  "sheep"
    set color white
  ...
  create-sheep_b initial-number-sheep_b  ; create the sheep b, then initialize their variables
  [
    set shape  "sheep"
    set color yellow
    set size 1.5  ; easier to see
...
to go
  ...
  if not any? wolves and (count sheep_a + count sheep_b) > max-sheep [ user-message "The sheep have inherited the earth" stop ]
  ask sheep_a [
    move
    ...
    reproduce-sheep_a  ; sheep reproduce at a random rate governed by a slider
  ]
  ...
  ask sheep_b [
    ...
    reproduce-sheep_b  ; sheep reproduce at a random rate governed by a slider
  ]
  ask wolves [
    move
...
to reproduce-sheep_a  ; sheep_a procedure
  if random-float 100 < sheep-reproduce [  ; throw "dice" to see if you will reproduce
    set energy (energy / 2) ; divide energy between parent and offspring
    ifelse random-float 100 < q [
      hatch-sheep_b 1 [ rt random-float 360 fd 1 ]   ; hatch an offspring (sheep b) and move it forward 1 step
    ][ 
      hatch-sheep_a 1 [ rt random-float 360 fd 1 ]   ; hatch an offspring (sheep a) and move it forward 1 step
    ]
  ]
end

to reproduce-sheep_b  ; sheep_b procedure
  if random-float 100 < sheep-reproduce [  ; throw "dice" to see if you will reproduce
    set energy (energy / 2)                ; divide energy between parent and offspring
    ifelse random-float 100 < q [
      hatch-sheep_a 1 [ rt random-float 360 fd 1 ]   ; hatch an offspring (sheep a) and move it forward 1 step
...
to eat-sheep  ; wolf procedure
  let prey1 one-of sheep_a-here                    ; grab a random sheep
  let prey2 one-of sheep_b-here
  
  let prey nobody
  
  ifelse count sheep_a-here = 0 and count sheep_b-here = 0 [
    set prey nobody
  ] [
    ifelse count sheep_a-here = 0 [ 
    	set prey prey2
    ] [ 
    	ifelse count sheep_b-here = 0 [
      	set prey prey1
      ] [
        ifelse random-float 1 < (count sheep_a-here / ((count sheep_b-here) + (count sheep_a-here))) [
          set prey prey1
        ] [
          set prey prey2
        ]
...
to display-labels
  ...
  if show-energy? [
    ...
    if model-version = "sheep-wolves-grass" [ 
      ask sheep_a [ set label round energy ] 
      ask sheep_b [ set label round energy ] 
    ]
  ]
end
...
"" ""
PENS
"sheep A" 1 0 -612749 true "" "plot count sheep_a"
...
"sheep B" 1 0 -6859420 true "" "plot count sheep_b"
