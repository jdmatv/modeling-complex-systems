...
globals [
  ...
  houses-per-district ;
  count-dict ; table in https://davidlowryduda.com/segregation-gerrymandering-and-schellings-model/
  district-id-list ; list of unique district id's
  wasted-votes ;
  efficiency-gap ;
]
...
turtles-own [
  ...
  my-district		   ; district id of turtle
]

breed [distmarkers distmark]
...
to setup
  ...
  ; We could use "resize-world min-pxcor max-pxcor min-pycor max-pycor" to change the world size and
  ; account for the introduction of district lines that occupy patches.
  ; This would allow us to keep the number of agentys constant when switching between scenarios

  ; create turtles on random patches.
  ask patches [

    set pcolor white
    ; Construct district lines and assign district-id to each squared district that groups a subset of patches
    set district-line 0
    if (pxcor mod district-size < 1) or (pycor mod district-size < 1) [
      set pcolor black
      set district-line 1
    ]
    set district-id  (2 * max-pxcor / district-size) * int((pycor + max-pycor) / district-size) + int( (pxcor + max-pxcor - 1) / district-size) + 1 ; function(x,y,N,size) return(floor(N/size)*(floor(y/size))+floor((x-1)/size)+1)

    if (random 100 < density) and (pcolor = white) [   ; set the occupancy density
      sprout 1 [
  ...
  set houses-per-district (district-size - 1) ^ 2
...
to find-new-spot
  rt random-float 360
  fd random-float 10
  if (any? other turtles-here) or (district-line = 1 ) [ find-new-spot ] ; keep going until we find an unoccupied patch
...
to update-turtles
  ask turtles [
    ...
    set my-district [ district-id ] of patch-here ; added a variable with the district id of the patch a turtle is on
...
; added function to create the table in https://davidlowryduda.com/segregation-gerrymandering-and-schellings-model/
to update-counts
  set district-id-list sort remove-duplicates [ district-id ] of patches with [ district-line = 0 ] ; generating unique list of district id's excluding boundaries]
  set count-dict table:make
  let net-wasted-n (list)
  (foreach district-id-list
    [ a ->
      let dis-count-blue (count turtles with [ color = 105 and my-district = a ] )
      let dis-count-orange (count turtles with [ color = 27 and my-district = a ] )
      let winner "blue"
      if dis-count-orange > dis-count-blue [ set winner "orange" ]
      let blue-wasted 0
      let orange-wasted 0
      if winner = "blue" [ 
        set blue-wasted dis-count-blue - (dis-count-orange + 1)
        set orange-wasted dis-count-orange
      ]
      if winner = "orange" [ 
        set orange-wasted dis-count-orange - (dis-count-blue + 1)
        set blue-wasted dis-count-blue
      ]
      let net-wasted blue-wasted - orange-wasted
    	set net-wasted-n lput net-wasted net-wasted-n
      let out-list (list dis-count-blue dis-count-orange winner blue-wasted orange-wasted net-wasted)
      table:put count-dict a out-list ] ; table in https://davidlowryduda.com/segregation-gerrymandering-and-schellings-model/
  )
  set wasted-votes sum net-wasted-n
  set efficiency-gap (wasted-votes / count turtles )
end
...
"" ""
PENS
"Pen 1" 1 0 -16579837 true "" "plot efficiency-gap * 100"
...
"" ""
PENS
"Pen 1" 1 0 -16579837 true "" "plot wasted-votes"
@#$#@#$#@
## WHAT IS IT?
